...
export function useSubscriptionCheck() {
  ...
  // Set current path only on client side to avoid hydration issues
  useEffect(() => {
  ...
  useEffect(() => {
    // Only run effects after component has mounted and data is available
    if (data && !isLoading && currentPath && typeof window !== 'undefined') {
  ...
  }, [data, router, isLoading, currentPath]);
